...
*/
FirstBossLeft::FirstBossLeft(	unsigned int initMin, 
...
*/
FirstBossLeft::FirstBossLeft(	Game *game,
										Physics *physics,
										...
										unsigned int initLevel)
{
	...
	BossBot1 = spriteManager->getSpriteType(10);
...
*/
void FirstBossLeft::initBot(			unsigned int initMaxVelocity)
{
}
...
*/
void FirstBossLeft::pickRandomVelocity()
{
	}
...
*/
void FirstBossLeft::think(Game *game)
{
	...
	b2Body *bossLeftBody = this->getBody();
	int bossLeftWidth = this->getSpriteType()->getTextureWidth();
	int bossLeftHeight = this->getSpriteType()->getTextureHeight();
	bossLeftBody->SetTransform(b2Vec2(this->pp.getX()/3.0f + bossLeftWidth/4+10, this->pp.getY()/3.0f + bossLeftHeight/5+15), 0);
	this->setBody(bossLeftBody);
}
